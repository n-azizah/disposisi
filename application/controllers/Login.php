...
$this->session->set_userdata($data);
 ...
 if ($user['role'] == 'admin') {
     ...
     redirect('admin/menu');
} else if ($user['role'] == 'sekretaris') { $this->_updateLastLogin($userid);
    redirect('surat');
 }
} else {
    //jika password salah
...
-danger" role="alert"> <b>Error :</b> User Tidak Terdaftar. </div>'); redirect('/');
} }
private function _updateLastLogin($userid){
$sql = "UPDATE tb_user SET last_login=now() WHERE id=$userid"; $this->db->query($sql);
}
...
{
$data = array(
'user' => infoLogin(), 'title' => 'Access Denied!'
);
